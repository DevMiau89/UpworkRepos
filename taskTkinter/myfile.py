...
def exportPage(filePath):
    ...
    html = """<div style="width: 80%%; margin: 0 auto;"> %s %s %s The data was sourced from: <a href=%s>%s</a></div> """ % (h1, img, tb, outputFilename.replace("Archive/",""), outputFilename)
...
class PageListOneCurrent(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tableTest = re.findall(r'<table(.*?)</table>', res.text, re.M | re.I | re.S)[0]
...
class PageListTwoPrevious(tk.Frame):
    def __init__(self, parent, controller):
        ...
        res2 = requests.get(urlTwo)
        tableTest = re.findall(r'<table(.*?)</table>', res2.text, re.M | re.I | re.S)[0]
        tdTest = re.findall(r'<td(.*?)</td>', tableTest, re.M | re.I | re.S)
